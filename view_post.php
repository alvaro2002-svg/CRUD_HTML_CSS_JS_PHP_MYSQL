...
<body>
    ...
    <section class="view-post">
        <div class="heading">
            <h1>post details</h1>
            <a href="all_posts.php" style="margin-top: 0;" class="inline-option-btn">all posts</a>
        </div>

        <?php
        ## Selecciona el posts segun el ID que le de la URL. 
        $select_post = $conn->prepare("SELECT * FROM `posts` WHERE id = ? LIMIT 1");
        $select_post->execute([$get_id]);
        ## si el post seleccionado tiene algo
        if ($select_post->rowCount() > 0) {
            ## Mira los posts asociados
            while ($fetch_post = $select_post->fetch(PDO::FETCH_ASSOC)) {
                ...
                while ($fetch_ratign = $select_ratings->fetch(PDO::FETCH_ASSOC)) {
                    $total_ratings += $fetch_ratign["rating"];
                    if ($fetch_ratign["rating"] == 1) {
                        $rating_1 += $fetch_ratign["rating"];
                    }
                    if ($fetch_ratign["rating"] == 2) {
                        $rating_2 += $fetch_ratign["rating"];
                    }
                    if ($fetch_ratign["rating"] == 3) {
                        $rating_3 += $fetch_ratign["rating"];
                    }
                    if ($fetch_ratign["rating"] == 4) {
                        $rating_4 += $fetch_ratign["rating"];
                    }
                    if ($fetch_ratign["rating"] == 5) {
                        $rating_5 += $fetch_ratign["rating"];
                    }
                }
                ## total reseñas
                if ($total_reviews != 0) {
                    ## Promedio
                    $average = round($total_ratings / $total_reviews);
                } else {
                    $average = 0;
                }
        ?>
                <!-- Muestra los que tenemos en pantalla -->
                <div class="row">
                    <div class="col">
        ...
        <?php
            ...
            echo '<p class="empty">posts is missing!</p>';
        }
        ?>
    </section>
    ...
    <section class="review-container">
        <div class="heading">
            ...
            <a href="add_review.php?get_id=<?= $get_id; ?>" style="margin-top: 0;" class="inline-btn">add reviews</a>
        </div>
